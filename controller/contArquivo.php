<?php
    ...
    function export(){
		try{
			...
			//$cabecalho  = "Dia_Semana,Periodo,Ocorrencia,Feriado,automovel,bicicleta,caminhao,caminhonete,moto,onibus,pedestre";
			...
			echo '<h3>Não foi possivel conectar no banco de dados.</h3>';
		}
	...
	function verificarFeriado($dataHora){
		...
		try{
	        ...
	        foreach ($dados as $document){
				...
				// semana comeca na segunda, quarta é dia 3. A semana comeca no dia 1
				if ($diaSem < 3){ // Feriado no inicio da semana, considera feriadao do dia do feriado até dia 1
					$qtdeDias = $diaSem + 1;
					...
					if ($diaSem > 3){ // Feriado no Fim da semana, condidera feriado do dia 7 até o dia do feriado
						$qtdeDias = 7 - $diaSem;
	...
	function verificarVeiculo($texto){
		...
		/* Verifica se algum automóvel se envolveu */
		...
		/* Verifica se alguma bicicleta está envolvida */
		...
		/* Verifica se algum caminhão está envolvido */
		...
		/* Verifica se alguma caminhonete está envolvida */
		...
		/* Verifica se alguma moto está envolvida */
		...
		/* Verifica se algum onibus está envolvido */
		...
		/* Verifica se algum pedestre está envolvido */
